...
TYPE_CHART = {
    "불": {"풀": 2.0, "불": 0.5, "물": 0.5, "어둠": 2.0, "얼음": 2.0},
    ...
    "얼음": {"풀": 2.0, "땅": 2.0, "비행": 2.0, "물": 0.5, "불": 0.5, "얼음": 0.5},
...
SKILL_DATABASE = {
    ...
    "얼음": {
        "하급": [
            {"name": "얼음송곳", "mp": 20, "power": 20},
            {"name": "몸통박치기", "mp": 10, "power": 10},
            {"name": "나인테일", "mp": 20, "power": 20}
        ],
        "중급": [
            {"name": "아이스 스피어", "mp": 30, "power": 40},
            {"name": "얼음방패", "mp": 25, "power": 30},
            {"name": "우박 던지기", "mp": 35, "power": 40}
        ],
        "상급": [
            {"name": "얼음 회오리", "mp": 50, "power": 80},
            {"name": "얼음 폭풍", "mp": 60, "power": 90},
            {"name": "스킬 정지", "mp": 50, "power": 50}
        ],
        "궁극기": [
            {"name": "절대영도", "mp": 80, "power": 120}
        ]
    },
    "어둠": {
        "하급": [
...
class PvPBattle:
    ...
    def process_attack(self, attacker, defender, chosen_skill_name, is_a=True):
        ...
        if attacker.main_type == "얼음" and random.random() < 0.10:
            if is_a and self.status_b != "동결":
                self.status_b = "동결"
                self.log.append(f"❄️ {defender.name}에게 [동결]이 부여되었습니다! (행동 불가)")
            elif not is_a and self.status_a != "동결":
                self.status_a = "동결"
                self.log.append(f"❄️ {defender.name}에게 [동결]이 부여되었습니다! (행동 불가)")
