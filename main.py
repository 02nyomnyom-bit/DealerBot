...
class EnhancedBot(commands.Bot):
    ...
    async def update_daily_status(self):
        ...
        await self.change_presence(activity=discord.CustomActivity(name=f"📅{date_str} | {greeting} | (v1.12.9)"), status=discord.Status.online)
        ...
        if guild.id not in Config.MAIN_GUILD_IDS:
            ...
            try:
                if guild.system_channel:
                    ...
                    embed.set_footer(text="보석상 v1.12.9 - 서버 제한 시스템")
    ...
    async def on_ready(self):
        ...
        print("=" * 50)
        print(f"🎉 보석상(v1.12.9) 시스템 가동 성공 (60초 후 백그라운드 명령어 동기화 예정)")
...
async def main():
    ...
    try:
        logger.info("🚀 보석상 v1.12.9 향상된 업데이트 시스템 시작 중...")
